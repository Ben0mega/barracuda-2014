...
>>>>>>> 2d3af12b327fa85b93a70c50a3dfdbf779155889
           return True 
...
<<<<<<< HEAD
    if not isLastCard(msg) and calculateHandScore(msg, 9.5):
    ...
    if (msg["state"]["your_points"] - msg["state"]["their_points"]) > neededToWin(msg):
        if calculateHandScore(msg, 9.7):
            return True
    if msg["state"]["your_points"] == 9:
        if calculateHandScore(msg, 11):
=======
...
>>>>>>> 2d3af12b327fa85b93a70c50a3dfdbf779155889
    ...
    if msg["state"]["their_tricks"] < neededToWin(msg):    # can you win the challenge?
        if msg["state"]["their_tricks"] == (neededToWin(msg)-1) and "card" in msg["state"].keys() and msg["state"]["card"] > max(msg["state"]["hand"]):
            return False
<<<<<<< HEAD
        if calculateHandScore(msg, 10.2) and msg["state"]["their_tricks"]+count_num_card(msg, 7) < neededToWin(msg):
=======
...
def shouldAcceptChallenge(msg, deck):
    ...
    if msg["state"]["your_tricks"]+count_num_card(msg, 12) >=3:
         return True
<<<<<<< HEAD
    if calculateHandScore(msg, 10.2) and len(msg["state"]["hand"]) > 1:
=======
